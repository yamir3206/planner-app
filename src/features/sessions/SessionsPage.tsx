...
export function SessionsPage() {
  ...
  const sortedSessions = React.useMemo(() => {
    return [...sessions].sort((a, b) => {
      try { return new Date(b.startTime).getTime() - new Date(a.startTime).getTime() }
      catch { return 0 }
    })
  }, [sessions])
  ...
  const handleManualSubmit = async () => {
    ...
    try {
      ...
      if (editingId) await sessionService.delete(editingId)
    ...
    <PageContainer>
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" onClick={() => { setEditingId(null); setShowManual(true) }}>
          <Plus className="h-4 w-4" />ثبت دستی مطالعه
        </Button>
      </div>
      ...
      <Card className="border-primary/20 bg-primary/5">
        <CardContent className="p-4">
          ...
          <p className="text-xs text-muted-foreground">اگر قبلاً مطالعه کردی و تایمر نزدی، اینجا دستی وارد کن تا در آمار حساب بشه.</p>
        </CardContent>
      </Card>

      {sortedSessions.length === 0 ? (
        <Card><CardContent><EmptyState icon={History} title="جلسه‌ای ثبت نشده" description="از تایمر استفاده کن یا دستی ثبت کن" action={{ label: 'ثبت دستی', onClick: () => setShowManual(true) }} /></CardContent></Card>
      ) : (
                      ...
                      <Card key={session.id} className="group">
                        <CardContent className="p-4 flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            <div className="h-10 w-10 rounded-xl flex items-center justify-center text-white" style={{ backgroundColor: subject?.color || '#6366F1' }}><Clock className="h-5 w-5" /></div>
                            <div>
                              <p className="font-medium flex items-center gap-2">{subject?.name || 'نامشخص'}<Badge variant="outline" className="text-xs">{session.type === 'pomodoro' ? '🍅 پومودورو' : session.type === 'free' ? '📚 آزاد' : '⏱️ معکوس'}</Badge></p>
                              <p className="text-xs text-muted-foreground">{(() => { try { return new Date(session.startTime).toLocaleTimeString('fa-IR', { hour: '2-digit', minute: '2-digit' }) } catch { return '--:--' } })()} • {formatDuration(session.duration, settings.persianNumbers, false)}</p>
                            </div>
                          </div>
                          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleEdit(session)}><Edit2 className="h-4 w-4" /></Button>
                            <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => { if (confirm('حذف جلسه؟')) remove(session.id) }}><Trash2 className="h-4 w-4" /></Button>
                          </div>
                        </CardContent>
                      </Card>
                    )
      ...
      <Modal open={showManual} onClose={() => { setShowManual(false); setEditingId(null) }} title={editingId ? 'ویرایش جلسه' : 'ثبت دستی جلسه مطالعه'} size="lg">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Select label="نوع جلسه" value={manualForm.type} onChange={e => setManualForm({ ...manualForm, type: e.target.value as any })} options={[{ value: 'free', label: 'آزاد' }, { value: 'pomodoro', label: 'پومودورو' }, { value: 'countdown', label: 'معکوس' }]} />
          </div>
          <Textarea label="یادداشت" value={manualForm.notes} onChange={e => setManualForm({ ...manualForm, notes: e.target.value })} />
        </div>
        <ModalFooter>
          <Button variant="secondary" onClick={() => { setShowManual(false); setEditingId(null) }}>انصراف</Button>
          <Button onClick={handleManualSubmit}>{editingId ? 'ذخیره' : 'ثبت'}</Button>
        </ModalFooter>
      </Modal>
    </PageContainer>
  )
